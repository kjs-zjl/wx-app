  ...
  methods: {
    ...
    refresh () {
      ...
      Promise.all([this.getLocationList(), this.getCommingList()]).then((results) => {
        this.locationList = results[0].ms.slice(2, 11)
        this.commingList = results[1].moviecomings.slice(2, 11)
    ...
    load () {
      this.loading = true
      Promise.all([this.getLocationList(), this.getCommingList()]).then((results) => {
        this.locationList = results[0].ms.slice(11, 20)
        this.commingList = results[1].moviecomings.slice(11, 20)
        this.loading = false
